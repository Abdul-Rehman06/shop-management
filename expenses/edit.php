...
$existingSplits = exp_active_payment_splits($pdo, $id);
$paymentAccountIds = [];
$paymentSplitAmounts = [];
foreach ($existingSplits as $split) {
    $paymentAccountIds[] = (int) ($split['payment_source_account_id'] ?? 0);
    $paymentSplitAmounts[] = number_format((float) ($split['amount'] ?? 0), 2, '.', '');
}
if (!$paymentAccountIds) {
    $fallbackAccountId = (int) ($row['payment_source_account_id'] ?? 0);
    $paymentAccountIds = [$fallbackAccountId > 0 ? $fallbackAccountId : $defaultCashAccountId];
    $paymentSplitAmounts = [$paymentStatus === 'paid' ? number_format((float) $amount, 2, '.', '') : ''];
}
$error = '';
    ...
    if ($date === '') {
        ...
        if ($error === '') {
            ...
            try {
                ...
                if ($paymentStatus === 'paid') {
                    $snapshot = exp_apply_payment_history($pdo, $id, [
                        'bill_name' => $billName,
                        'category' => $category,
                    ...
                    exp_reverse_payment_history($pdo, $id, $paidBy !== '' ? $paidBy : null);
                }
