...
$pageTitle = 'Add Purchase - Load Management';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($date === '') {
        ...
        $stmt = $pdo->prepare('
            ...
            VALUES
                (:date, :network, :type, 0, :purchased, 0, NULL, 0, 0, :supplier, :remarks)
        ');
        $stmt->execute([
            ':date' => $date,
            ':network' => $network,
            ':type' => 'purchase',
            ':purchased' => (float) $amount,
            ':supplier' => $supplier !== '' ? $supplier : null,
            ':remarks' => $remarks !== '' ? $remarks : null,
        ]);

        load_recalculate_network($pdo, $network);
        flash_set('success', 'Purchase entry added successfully.');
...
<div class="d-flex align-items-center justify-content-between mb-3">
    <h1 class="h4 mb-0">Add Purchase</h1>
