...
$totals = $accountId > 0 ? jc_totals($pdo, $accountId) : ['opening' => 0.0, 'receiving' => 0.0, 'sending' => 0.0, 'closing' => 0.0, 'commission' => 0.0];
$searchTotals = null;
if ($accountId > 0 && $q !== '') {
...
<div class="d-flex flex-wrap gap-2 align-items-center justify-content-between mb-3">
    <div class="d-flex align-items-center gap-3 flex-wrap">
        ...
        <?php if ($isOwner): ?>
            <a class="btn btn-outline-secondary btn-sm" href="<?= h(app_url('settings/accounts.php?type=jazzcash')) ?>">Manage Accounts</a>
        <?php endif; ?>
    </div>
    <div class="d-flex flex-wrap gap-2">
        <a class="btn btn-secondary btn-sm" href="<?= h(app_url('jazzcash/report.php')) ?>">Reports</a>
        <a class="btn btn-outline-secondary btn-sm" href="<?= h(app_url('jazzcash/opening.php?account_id=' . $accountId)) ?>">Set Opening</a>
        <a class="btn btn-outline-primary btn-sm" href="<?= h(app_url('jazzcash/receiving.php?account_id=' . $accountId)) ?>">Add Receiving</a>
        <a class="btn btn-gradient shadow-glow btn-sm" href="<?= h(app_url('jazzcash/sending.php?account_id=' . $accountId)) ?>">Add Sending</a>
    </div>
</div>
        ...
        <div class="card-body">
            <form method="get" class="row g-2 align-items-end">
                ...
                <div class="col-12 col-md-auto d-flex gap-2">
                    ...
                    <a class="btn btn-outline-secondary" href="<?= h(app_url('jazzcash/index.php?account_id=' . $accountId)) ?>">Clear</a>
                </div>
            </form>
        </div>
    </div>
...
<?php if (!$accounts): ?>
    <div class="alert alert-warning">No JazzCash accounts found. Add accounts first.</div>
<?php else: ?>
    <div class="row g-3 mb-3">
